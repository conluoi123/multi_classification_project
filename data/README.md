...
1. **Tải dữ liệu EUR-Lex gốc:** Truy cập kho dữ liệu [Extreme Classification Repository (XMLRepository)](http://manikvarma.org/downloads/XC/XMLRepository.html), tải bộ `Eurlex-4K`.
2. **Giải nén vào thư mục data:** Đặt file đã giải nén (ví dụ: `eurlex_train.txt` và `eurlex_test.txt`) vào thư mục `data/`.
3. **Cập nhật đường dẫn trong kịch bản:** Mở `src/experiments/exp_real_datasets.py` và truyền đường dẫn file vào hàm `load_eurlex_data(file_path='data/eurlex_train.txt')`.
